...
export class SeedSlingshot {
 ...
 begin(seed){if(this.active)this.cancel();const b=seed.body;this.active={seed,position:{...b.translation()},rotation:{...b.rotation()},velocity:{...b.linvel()},angular:{...b.angvel()}};this.pull.set(0,0,0);b.setBodyType(this.R.RigidBodyType.Fixed,true);b.resetForces(false);b.resetTorques(false);}
 aim(point){if(!this.active)return;this.pull.copy(point).sub(this.active.position);this.pull.y=0;this.pull.clampLength(0,MAX_PULL);}
 release(){if(!this.active)return false;const {seed}=this.active,velocity=launchVelocity(this.pull);if(!velocity.lengthSq()){this.cancel();return false;}seed.body.setBodyType(this.R.RigidBodyType.Dynamic,true);seed.body.setLinvel(velocity,true);seed.body.setAngvel({x:velocity.z*3,y:0,z:-velocity.x*3},true);this.active=null;return true;}
 cancel(){if(!this.active)return;const s=this.active,b=s.seed.body;b.setBodyType(this.R.RigidBodyType.Dynamic,true);b.setTranslation(s.position,true);b.setRotation(s.rotation,true);b.setLinvel(s.velocity,true);b.setAngvel(s.angular,true);this.active=null;this.pull.set(0,0,0);}
}
