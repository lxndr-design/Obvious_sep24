...
function splash(p,amount=8){water.splash((p.x-.5)/5,(p.z+4)/5,amount);notify(state.paused?'Ripple queued — resume to see it travel':'Drag through the water to leave a wake');}
...
function tick(now){
 ...
 if(!state.paused){
  ...
  waterGeometry.attributes.position.needsUpdate=true;waterGeometry.computeVertexNormals();poolSpray.update();
 }
