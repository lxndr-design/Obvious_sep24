...
import {BirdWaterContacts} from './bird-water.js';
...
export class Ecology {
 ...
 update(delta,camera,width,height){
  ...
  if(!this.feedingMode&&camera&&this.pointerScreen)for(const bird of this.colony.birds){const projected=bird.position.clone().project(camera),x=(projected.x+1)*width/2,y=(1-projected.y)*height/2;if(Math.hypot(x-this.pointerScreen.x,y-this.pointerScreen.y)<60)this.colony.scare(bird,this.pointer);}
  if(!this.feedingMode&&camera&&this.pointerScreen&&this.ducks.ducks.some(duck=>{const p=duck.position.clone().project(camera);return Math.hypot((p.x+1)*width/2-this.pointerScreen.x,(1-p.y)*height/2-this.pointerScreen.y)<65;}))this.ducks.scare(this.pointer);
  ...
  for(const duck of this.ducks.ducks){let view=this.duckViews.get(duck.id);if(!view){view=duckMesh(duck.variant);this.duckViews.set(duck.id,view);this.group.add(view.group);}view.group.position.copy(duck.position);poseDuckGait(view,duck,delta);view.group.scale.setScalar(duck.scale);for(const m of view.materials)m.opacity=duck.opacity;view.group.traverse(o=>{if(o.isMesh)o.castShadow=duck.opacity>.7;});}
  ...
  for(const bird of this.colony.birds){let view=this.birdViews.get(bird.id);if(!view){view=birdMesh(BIRD_PALETTES[(bird.id-1)%BIRD_PALETTES.length]);this.birdViews.set(bird.id,view);this.group.add(view.group);}view.group.position.copy(bird.position);view.group.rotation.y=bird.yaw;view.group.scale.setScalar(bird.scale??1);view.body.rotation.z=-bird.peck*.52;
   attachCarriedStick(view,bird);setBirdWings(view,bird.wingSpread,bird.wing);setBirdFatness(view,view.fatness+(bird.fatness-view.fatness)*(1-Math.exp(-8*delta)),bird.headBob??0);for(const m of view.materials)m.opacity=bird.opacity;view.group.traverse(o=>{if(o.isMesh)o.castShadow=bird.opacity>.7;});
  }
 }
