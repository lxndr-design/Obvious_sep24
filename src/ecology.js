...
export class Ecology {
 ...
 update(delta,camera,width,height){
  ...
  if(!this.feedingMode&&camera&&this.pointerScreen&&this.ducks.ducks.some(duck=>{const p=duck.position.clone().project(camera);return Math.hypot((p.x+1)*width/2-this.pointerScreen.x,(1-p.y)*height/2-this.pointerScreen.y)<65;}))this.ducks.depart(this.pointer);
