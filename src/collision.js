...
export class CollisionScene {
 ...
 setTerrain(layout){this.layout=layout;this.voids=layout.holes.map(h=>({shape:new this.R.Cuboid(h.size/2,.3,h.size/2),position:{x:h.x,y:-.28,z:h.z}}));}
 ...
 canPlace(object,position,quaternion=object.mesh.quaternion){
  ...
  for(const a of object.parts){const ap=this.position(a,position,quaternion);
   for(const hole of this.voids){const contact=a.shape.contactShape(ap,quaternion,hole.shape,hole.position,identity,0);if(contact&&contact.distance<-.001)return false;}
 ...
 canTravel(object,to){if(!this.canPlace(object,to))return false;const from=object.mesh.position,velocity=to.clone().sub(from);if(velocity.lengthSq()<1e-8)return true;
  for(const a of object.parts){const ap=this.position(a,from,object.mesh.quaternion);
   ...
   for(const hole of this.voids)if(test(hole.shape,hole.position,identity))return false;
 ...
 contactPosition(object,to){
  ...
  for(const a of object.parts){const ap=this.position(a,from,object.mesh.quaternion);
   ...
   for(const hole of this.voids)test(hole.shape,hole.position,identity);
