...
- **Pool** adds a 2 m square water-filled opening. Drag its edge or use arrow keys to move it; Remove/Delete fills it back in. The original 5 m pool is also a pool object. Touching or overlapping squares share one opening, with no interior walls and waves crossing the join. Each square remains individually editable. Pools cannot be placed through supporting solid geometry. Forms can be dragged into the water to rest on the basin bottom, then lifted back over the rim onto dry ground. Wide forms remain supported wherever they overlap the rim.
...
- Pull back from a spiked seed pod to aim, then release to launch it in the opposite direction. A line shows the pull and an arrow shows the launch direction; longer pulls launch harder, with a maximum pull of 2.5 m. Escape, a cancelled pointer, or switching away cancels the launch.
- Birds arrive in blue, rust, gold, charcoal, and white plumage, with darker wings. Shaded dithering preserves their colors; two-tone mode uses only the selected ink/paper palette.
...
**Ground and lighting.** White ground patches surround the union of editable recessed holes and extend 4,096 m in every direction. Rendering, collision walls, and water regions rebuild together when a pool changes. There is no raised slab. Nearby physics patches are kept small for stable contact with tiny seed spikes, with outer patches continuing the ground. Solid objects use pure white base color; water has a translucent gray tint, with neutral lighting and a white-paper default palette; shading and shadows provide their definition. One hemisphere light, one directional light, one 2048² shadow map and one Bayer dithering postprocess keep rendering simple. Shadows follow panning/zooming; meadow shadows refresh at 30 Hz. No bloom, ambient-occlusion pass or image blur is used.
