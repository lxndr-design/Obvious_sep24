...
const scene=new CollisionScene(R);let contacts=0,casts=0;
...
function run(name,target){const times=[];contacts=casts=0;for(let i=0;i<120;i++){bath.mesh.position.copy(origin);const start=performance.now();scene.move(bath,target(i));times.push(performance.now()-start);}times.sort((a,b)=>a-b);console.log(JSON.stringify({name,meanMs:+(times.reduce((a,b)=>a+b,0)/times.length).toFixed(3),p95Ms:+times[114].toFixed(3),contactsPerMove:Math.round(contacts/120),castsPerMove:Math.round(casts/120)}));}
for(let i=0;i<5;i++)scene.move(bath,new THREE.Vector3(7,bath.height/2,0));
