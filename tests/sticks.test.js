...
test('birds arrive for sticks without leaves, dip to pick up, then fly and fade with the stick',()=>{
 const o=stick(),objects=[o],sticks=new StickCollection(()=>objects),colony=new BirdColony(seededRandom(6));sticks.onTake=()=>objects.splice(0,1);let sawPickup=false,sawFlight=false,restTime=0;
 advance(dt=>{colony.step(dt,sticks.sites());for(const b of colony.birds){if(b.state==='collecting'&&b.peck>.2)sawPickup=true;if(b.carriedStick&&b.state==='lingering'){restTime+=dt;assert.equal(b.opacity,1);}if(b.carriedStick&&b.state==='departing'&&b.position.y>1&&b.opacity>0&&b.opacity<1){sawFlight=true;assert.equal(b.wingState,'flapping');}}},28);
 assert.ok(sawPickup);assert.ok(restTime>=6);assert.ok(sawFlight);assert.equal(objects.length,0);assert.equal(colony.birds.length,0);assert.deepEqual([...sticks.taken],[1]);
});
