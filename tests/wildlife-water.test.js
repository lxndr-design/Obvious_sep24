...
test('live ecology renders a duck pair and couples bird contacts to bath water even without bathing spray',()=>{
 ...
 for(let i=0;i<26*60;i++){p.step(1/60);ecology.update(1/60,null,0,0);terrain.step(1/60,wind);bathPeak=Math.max(bathPeak,...ecology.bathViews.get(bath).field.height.map(Math.abs));poolPeak=Math.max(poolPeak,...terrain.views[0].field.height.map(Math.abs));}
 ...
 for(const d of ecology.ducks.ducks){const v=ecology.duckViews.get(d.id);assert.ok(v.group.position.distanceTo(d.position)<1e-8);assert.equal(v.feet[0].visible,!d.swimming);}
