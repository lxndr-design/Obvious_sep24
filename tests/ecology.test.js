...
test('nearby mouse scares birds, fades them out, and prevents immediate respawn',()=>{const c=new BirdColony(),p=pile();advance(dt=>c.step(dt,[p]),12);assert.ok(c.birds.some(b=>b.state==='foraging'));c.disturb(new THREE.Vector3(0,0,0),[p]);assert.ok(c.birds.every(b=>b.fleeAt>c.time));advance(dt=>c.step(dt,[p],p.position),2.2);assert.ok(c.birds.every(b=>b.state==='departing'));assert.ok(c.birds.every(b=>b.opacity<1));advance(dt=>c.step(dt,[p],p.position),10);assert.equal(c.birds.length,0);});
