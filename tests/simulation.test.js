...
test('open ground permits distant placements while the pool opening rejects floor placements',()=>{const p=new CollisionScene(R),o=object('sphere',-3,2);assert.equal(p.canPlace(o,new THREE.Vector3(-70,.75,20)),true);assert.equal(p.canPlace(o,new THREE.Vector3(3,.75,-1)),false);assert.equal(p.canPlace(o,new THREE.Vector3(3,3,-1)),true);});
